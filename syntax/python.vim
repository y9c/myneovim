...
if s:python36
  ...
  "      \ contains=pyVarTypeAnnotation
  syn match pyVarTypeOnly /\s*:/ nextgroup=@pyExpr skipwhite display
        \ contains=pyVarTypeAnnotation
  syn match pyVarTypeAnnotation contained /:\s*.*\ze\s*[=$]/ contains=pyVarTypeColon
  syn match pyVarTypeColon contained /:/
  syn cluster pyClStatements add=pyVarTypeOnly
  hi! link pyVarTypeAnnotation Comment
...
if s:python34
  syn match pyDefType /\s*->/ contained nextgroup=@pyTypeExpr skipwhite display
              \ contains=pyDefTypeAnnotation
  syn match pyDefTypeAnnotation contained /->\s*.*/ contains=pyDefTypeArrow
  syn match pyDefTypeArrow contained /->/
  hi! link pyDefType pyTypeAnnotation
  hi! link pyDefTypeAnnotation Comment
endif
...
syn keyword pyKnownModule contained __builtin__ i18n
			\ httplib urllib2
			\ zlib gzip
			\ UserDict
...
if s:python34
    syn keyword pyKnownFunction contained print
endif
" special
...
hi! link Statement Macro
hi! link pyClStatements Error
" hi! link Macro Error
